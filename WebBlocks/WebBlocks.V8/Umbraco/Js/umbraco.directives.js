...
**/
    ...
    (function () {
        'use strict';
        function ContentEditController($rootScope, $scope, $routeParams, $q, $timeout, $window, $location, appState, contentResource, entityResource, navigationService, notificationsService, angularHelper, serverValidationManager, contentEditingHelper, treeService, fileManager, formHelper, umbRequestHelper, keyboardService, umbModelMapper, editorState, $http, eventsService, relationResource, overlayService, localizationService) {
            var evts = [];
            //setup scope vars
            ...
            function init(content) {
                ...
                if ($scope.editors.length === 0) {
                    var editor = { content: $scope.content };
         ...
         */
            function loadContent(culture) {
                ...
                $scope.getMethod()($scope.contentId, culture).then(function (data) {
                    $scope.content = data;
            ...
            $scope.unPublish = function () {
                ...
                if (formHelper.submitForm({
                    ...
                    $scope.page.buttonGroupState = 'busy';
                    contentResource.unPublish($scope.content.id, culture).then(function (data) {
    ...
    function umbTreeDirective($compile, $log, $q, $rootScope, treeService, notificationsService, $timeout, userService) {
        return {
            ...
            compile: function (element, attrs) {
                ...
                template += '<ul>' + '<umb-tree-item ng-repeat="child in tree.root.children" enablelistviewexpand="{{enablelistviewexpand}}" node="child" current-node="currentNode" tree="this" section="{{section}}" ng-animate="animation()"></umb-tree-item>' + '</ul>' + '</li>' + '</ul>';
...
.css('padding-left', node.level * 20 + 'px');
            ...
            */
                ...
                scope.loadChildren = function (node, forceReload) {
                    //emit treeNodeExpanding event, if a callback object is set on the tree
                    umbTreeCtrl.emitEvent('treeNodeExpanding', {
                        tree: scope.tree,
                        node: node
                    });
